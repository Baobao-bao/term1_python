...
class EditStaffPopup(tk.Frame):
    ...
    def __init__(self, parent, close_callback, id):
        ...
        ttk.Label(self, text="Position:").grid(row=6, column=1)
        self._position = ttk.Entry(self)
        self._position.grid(row=6, column=2)
    ...
    def _submit_cb(self):
        ...
        data['position'] = self._position.get()

        self.update_staff(data)

    def update_staff(self, data):
        """Makes post request"""
        requests.put("http://127.0.0.1:5000/school/update_staff/" + str(self.id), json=data)
        self._close_cb()
