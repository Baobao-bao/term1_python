...
class EditTeacherPopup(tk.Frame):
    ...
    def __init__(self, parent, close_callback, id):
        ...
        ttk.Label(self, text="Salary:").grid(row=6, column=1)
        self._salary = ttk.Entry(self)
        self._salary.grid(row=6, column=2)
    ...
    def _submit_cb(self):
        ...
        self.update_teacher(data)

    def update_teacher(self, data):
        """Makes post request"""
        requests.put("http://127.0.0.1:5000/school/update_teacher/" + str(self.id), json=data)
        self._close_cb()
